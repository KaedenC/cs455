...
def algo_2_update_locations(nodes):
    accels = []
    for i_agent in nodes:
        ...
        nf_term = 1.1 * np.array([i_agent.x - static_target[0], i_agent.y - static_target[1]])
...
def plot_network(nodes, iteration):
    ...
    update_gamma_agent(nodes)
    ax.plot(g_agent[0], g_agent[1], 'r>')
...
def update_gamma_agent(nodes):
    g_agent = center_of_mass(nodes)
    print(g_agent)
